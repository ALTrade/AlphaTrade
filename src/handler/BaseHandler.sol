...
contract BaseHandler is RoleModule, GlobalReentrancyGuard, OracleModule {
    ...
    constructor(RoleStore _roleStore, DataStore _dataStore, EventEmitter _eventEmitter, Oracle _oracle)
    ...
    function validateRequestCancellation(uint256 createdAtTime, string memory requestType) internal view {
    ...
    function validateNonKeeperError(bytes4 errorSelector, bytes memory reasonBytes) internal pure {
        if (
            OracleUtils.isOracleError(errorSelector) || errorSelector == Errors.DisabledFeature.selector
