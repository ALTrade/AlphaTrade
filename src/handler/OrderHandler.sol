...
contract OrderHandler is IOrderHandler, BaseOrderHandler {
    ...
    ) BaseOrderHandler(_roleStore, _dataStore, _eventEmitter, _oracle, _orderVault, _swapHandler, _referralStorage) {}
    ...
    function cancelOrder(bytes32 key) external override globalNonReentrant onlyController {
        ...
        FeatureUtils.validateFeature(
            _dataStore, Keys.cancelOrderFeatureDisabledKey(address(this), uint256(order.orderType()))
        );
    ...
    function simulateExecuteOrder(bytes32 key, OracleUtils.SimulatePricesParams memory params)
        external
        override
        onlyController
        ...
        globalNonReentrant
    {
        ...
        this._executeOrder(key, order, msg.sender);
    }
        ...
        withOraclePrices(oracleParams)
    {
        ...
        try this._executeOrder{gas: executionGas}(key, order, msg.sender) {}
    ...
    function _executeOrder(bytes32 key, Order.Props memory order, address keeper) external onlySelf {
        uint256 startingGas = gasleft();

        BaseOrderUtils.ExecuteOrderParams memory params =
            _getExecuteOrderParams(key, order, keeper, startingGas, Order.SecondaryOrderType.None);
    ...
    function _handleOrderError(bytes32 key, uint256 startingGas, bytes memory reasonBytes) internal {
        ...
        if (
            isMarketOrder || errorSelector == Errors.InvalidPositionMarket.selector
