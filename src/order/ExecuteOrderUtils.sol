...
library ExecuteOrderUtils {
    ...
    function executeOrder(BaseOrderUtils.ExecuteOrderParams memory params) external {
        ...
        MarketUtils.distributePositionImpactPool(
            params.contracts.dataStore, params.contracts.eventEmitter, params.market.marketToken
        );

        PositionUtils.updateFundingAndBorrowingState(
            params.contracts.dataStore, params.contracts.eventEmitter, params.market, prices
        );
        ...
        OrderEventUtils.emitOrderExecuted(
            params.contracts.eventEmitter, params.key, params.order.account(), params.secondaryOrderType
        );
        ...
        if (BaseOrderUtils.isDecreaseOrder(params.order.orderType())) {
            ...
            uint256 sizeInUsd =
                params.contracts.dataStore.getUint(keccak256(abi.encode(positionKey, PositionStoreUtils.SIZE_IN_USD)));
            if (sizeInUsd == 0) {
