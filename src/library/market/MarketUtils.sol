...
library MarketUtils {
    ...
    function validateEnabledMarket(
        DataStore dataStore,
        address marketAddress
    ) internal view {
        Market.Props memory market = MarketStoreUtils.get(
    ...
    function validateEnabledMarket(
        ...
        bool isMarketDisabled = dataStore.getBool(
    ...
    function validateSwapPath(
        ...
        uint256 maxSwapPathLength = dataStore.getUint(
            Keys.MAX_SWAP_PATH_LENGTH
        );
        if (swapPath.length > maxSwapPathLength) {
            revert Errors.MaxSwapPathLengthExceeded(
                swapPath.length,
                maxSwapPathLength
            );
        }
