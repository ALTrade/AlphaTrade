...
library IncreasePositionUtils {
    ...
    function increasePosition(PositionUtils.UpdatePositionParams memory params, uint256 collateralIncrementAmount)
        external
    {
        ...
        cache.collateralTokenPrice =
            MarketUtils.getCachedTokenPrice(params.position.collateralToken(), params.market, prices);
        ...
        (cache.priceImpactUsd, cache.priceImpactAmount, cache.sizeDeltaInTokens, cache.executionPrice) =
        ...
        (cache.collateralDeltaAmount, fees) = processCollateral(
            params, cache.collateralTokenPrice, collateralIncrementAmount.toInt256(), cache.priceImpactUsd
        );
        ...
        cache.nextPositionBorrowingFactor = MarketUtils.getCumulativeBorrowingFactor(
            params.contracts.dataStore, params.market.marketToken, params.position.isLong()
        );
        ...
        PositionUtils.updateOpenInterest(
            params, params.order.sizeDeltaUsd().toInt256(), cache.sizeDeltaInTokens.toInt256()
        );
        ...
        if (params.order.sizeDeltaUsd() > 0) {
            ...
            MarketUtils.validateOpenInterestReserve(
                params.contracts.dataStore, params.market, prices, params.order.isLong()
            );
