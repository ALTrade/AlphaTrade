...
library ExecuteDepositUtils {
    ...
    function executeDeposit(ExecuteDepositParams memory params, Deposit.Props memory deposit)
        external
        returns (uint256 receivedMarketTokens)
    {
        ...
        // minTimestamp should be greater than or equal to updatedAtTime
        // maxTimestamp should be less than or equal to updatedAtTime + requestExpirationTime
        if (params.oracle.minTimestamp() < deposit.updatedAtTime()) {
            revert Errors.OracleTimestampsAreSmallerThanRequired(params.oracle.minTimestamp(), deposit.updatedAtTime());
        }
        ...
        if (cache.maxOracleTimestamp > deposit.updatedAtTime() + cache.requestExpirationTime) {
            revert Errors.OracleTimestampsAreLargerThanRequestExpirationTime(
                cache.maxOracleTimestamp, deposit.updatedAtTime(), cache.requestExpirationTime
            );
        }

        cache.market = MarketUtils.getEnabledMarket(params.dataStore, deposit.market());

        // first deposit should be validated to ensure a minimum amount of liquidity is locked
        _validateFirstDeposit(params, deposit, cache.market);

        cache.prices = MarketUtils.getMarketPrices(params.oracle, cache.market);

        MarketUtils.distributePositionImpactPool(params.dataStore, params.eventEmitter, cache.market.marketToken);
    ...
    function _executeDeposit(ExecuteDepositParams memory params, _ExecuteDepositParams memory _params)
        internal
        returns (uint256)
    {
        ...
        MarketEventUtils.emitMarketPoolValueInfo(
            params.eventEmitter, params.key, _params.market.marketToken, poolValueInfo, marketTokensSupply
        );
        ...
        if (_params.priceImpactUsd > 0) {
            ...
            mintAmount += MarketUtils.usdToMarketTokenAmount(
                positiveImpactAmount.toUint256() * _params.tokenOutPrice.max, poolValue, marketTokensSupply
            );

            // deposit the token out, that was withdrawn from the impact pool, to mint market tokens
            MarketUtils.applyDeltaToPoolAmount(
                params.dataStore, params.eventEmitter, _params.market, _params.tokenOut, positiveImpactAmount
            );
        ...
        mintAmount += MarketUtils.usdToMarketTokenAmount(
            fees.amountAfterFees * _params.tokenInPrice.min, poolValue, marketTokensSupply
        );
        ...
        MarketUtils.validatePoolUsdForDeposit(
            params.dataStore, _params.market, _params.tokenIn, _params.tokenInPrice.max
        );
