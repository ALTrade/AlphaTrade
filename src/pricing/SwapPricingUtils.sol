...
library SwapPricingUtils {
        ...
        returns (int256)
    {
        ...
        if (isSameSideRebalance) {
            ...
            //如果是有利，priceImpactUsd为正数，如果是不利，priceImpactUsd为负数
            return PricingUtils.getPriceImpactUsdForSameSideRebalance(
                initialDiffUsd, nextDiffUsd, impactFactor, impactExponentFactor
            );
        } else {
            //平衡再交叉，需要同时获取positiveImpactFactor，negativeImpactFactor，之前有差距的那侧属于有利影响，
            //跨越了平衡点那侧属于不利影响，需要计算这两种影响的差值，来确定总体上是有利的还是不利的
    ...
    function getNextPoolAmountsParams(
        ...
        //如果是out的token，那么out的价值不能大于池子里面这种token的所有价值
    ...
    function emitSwapInfo(EventEmitter eventEmitter, EmitSwapInfoParams memory params) internal {
        ...
        eventEmitter.emitEventLog1("SwapInfo", Cast.toBytes32(params.market), eventData);
    }
