...
library FeeUtils {
    ...
    function incrementClaimableFeeAmount(
        ...
        uint256 nextValue = dataStore.incrementUint(key, delta);

        emitClaimableFeeAmountUpdated(eventEmitter, market, token, delta, nextValue, feeType);
    }
    ...
    function incrementClaimableUiFeeAmount(
        ...
        uint256 nextValue = dataStore.incrementUint(Keys.claimableUiFeeAmountKey(market, token, uiFeeReceiver), delta);

        uint256 nextPoolValue = dataStore.incrementUint(Keys.claimableUiFeeAmountKey(market, token), delta);

        emitClaimableUiFeeAmountUpdated(
            eventEmitter, uiFeeReceiver, market, token, delta, nextValue, nextPoolValue, feeType
        );
    }
    ...
    function claimFees(DataStore dataStore, EventEmitter eventEmitter, address market, address token, address receiver)
        internal
        returns (uint256)
    {
    ...
    function claimUiFees(
        ...
        uint256 nextPoolValue = dataStore.decrementUint(Keys.claimableUiFeeAmountKey(market, token), feeAmount);
    ...
    function emitClaimableFeeAmountUpdated(
        ...
        eventEmitter.emitEventLog2("ClaimableFeeAmountUpdated", Cast.toBytes32(market), feeType, eventData);
    }
    ...
    function emitClaimableUiFeeAmountUpdated(
        ...
        eventEmitter.emitEventLog2("ClaimableUiFeeAmountUpdated", Cast.toBytes32(market), feeType, eventData);
    }

    function emitFeesClaimed(EventEmitter eventEmitter, address market, address receiver, uint256 feeAmount) internal {
        EventUtils.EventLogData memory eventData;
        ...
        eventEmitter.emitEventLog1("FeesClaimed", Cast.toBytes32(market), eventData);
    }
    ...
    function emitUiFeesClaimed(
        ...
        eventEmitter.emitEventLog1("UiFeesClaimed", Cast.toBytes32(market), eventData);
    }
}
